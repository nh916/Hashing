...
//    private static final int DEFAULT_TABLE_SIZE = 101;
    ...
    private int[] occurenceOfLetters = new int[26];
    ...
    public int hash(String word, int gFirst, int gLast, int modValue){
        int hashValue = (word.length() + gFirst + gLast) % hashEntry.length;
        return hashValue;
    }
    public int hash(Key word, int gFirst, int gLast, int modValue){
        int hash = 0;
        hash = (word.size() + gFirst + gLast) % hashEntry.length;
        return hash;
    }
    public boolean cichelli(){
        populateOccurencesOfLetters();
        setValueOfAllKeys();
        ...
        return cichelliRec(keyWordStack, hashEntry, g, keys.size(),keys.size()/2);
    ...
    private  void populateOccurencesOfLetters(){
    ...
    //creates a smaller verison of the array created in previous method
    //it takes all of the letters in whcih the value was not 0, meaning that letters in a space with any other value are unique letters
    //they are the letters that appear more than once
    private void initializeUniqueLetters(){
        for(int i = 0; i < occurenceOfLetters.length; i += 1)	{
            if (occurenceOfLetters[i] != 0)	{
    ...
    private void setValueOfAllKeys()	{
        //set values of keys
        for(Key k : keys)	{
            int firstLetterValue = occurenceOfLetters[((int) k.getFirstLetter() - 97)];
            k.firstLetterValue = firstLetterValue;
            int lastLetterValue = occurenceOfLetters[((int) k.getLastLetter() - 97)];
